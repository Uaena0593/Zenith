...
const MyPortfolioGraph = () => {
    ...
    const changePortfolioStatus = async() => {
      setPortfolioStatus(!portfolioStatus);
      await axios.post('http://localhost:8000/change-portfolio-status', params: { queryStockSymbol: queryStockSymbol }, withCredentials : true);

    };
    return (
      <>
        <div className = 'flex flex-row search-bar-width justify-between'>
          ...
          <div className = 'flex flex-row items-center'>
            <div className = 'font-bold text-xl'>Public:</div>
            <Switch className = '' onChange = {()=>{changePortfolioStatus()}}/>
          </div>
        </div>
